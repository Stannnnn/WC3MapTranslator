...
export abstract class InfoTranslator {
    ...
    public static warToJson(
        buffer: Buffer,
        props?: { skipAfterMapInfo?: boolean }
    ): JsonResult<Info> {
        const result: Info = {
            map: {
                name: "",
        ...
        for (let i = 0; i < numPlayers; i++) {
            ...
            const isPlayerStartPositionFixed: boolean =
                outBufferToJSON.readInt() === 1; // 00000001 = fixed start position
